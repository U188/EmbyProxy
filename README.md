...
- 多 Emby 节点分流，支持节点密钥、多个独立中转视频线路、响应择优和失败切换
- 默认中转、媒体流直连、媒体流条件直连及请求 Header 策略
- Yamby、Hills Android、Hills Windows 客户端模拟
- 后台管理节点、测速、排序、导入导出、优选 IP 和 Cloudflare DNS
- 节点卡正面显示最近中转响应或直连重定向，点击翻转查看连接信息与高级操作
- D1 汇总每个节点和视频线路的真实 P50/P95、上游耗时、错误率及切换次数
- 访问日志分别记录入站客户端与出站模拟客户端、UA 和设备名
...
- 两个域名必须不同。配置后，管理域只开放后台/API，调度域只开放节点代理；`/api/health` 在两边都可用。
...
数据大屏的性能区域来自 Worker 真实代理请求，不是浏览器测速。系统按分钟汇总最近 7 天数据，展示请求量、P50/P95 响应建立耗时、平均上游耗时、错误率、线路切换次数，以及每条中转视频线路的成功率和响应延迟。系统不读取视频正文测速，视频响应体由 Cloudflare 原生流直接转发。统计只保存线路主机名和哈希标识，不保存完整上游路径。

节点的“独立视频线路”每行填写一个完整 `http://` 或 `https://` 地址。视频请求优先使用这些线路，普通 API 仍使用主上游。策略可选：

- `响应与健康择优`：根据实际请求的响应建立耗时排序，近期失败线路会暂时降权。
- `按填写顺序`：健康时保持配置顺序，失败时才切换备用线路。

“视频线路诊断”会在 5 秒内并行探测各线路，只有 `2xx/3xx` 才视为可用。多线路的 GET/HEAD 播放请求会按节点设置的 `500-10000ms` 响应头超时切换备用线路；收到 `403/404/408/429/5xx` 时也会尝试下一条线路。

播放方式只控制媒体流，登录和普通 Emby API 仍由 Worker 转发。`媒体流直连`始终把符合条件的播放请求重定向到上游；`媒体流条件直连`只在请求类型和鉴权允许安全携带时直链。系统不会读取视频正文测速，也不会根据播放速度在直连与中转之间切换。

如果非零 Range 拖动请求经过外部 CDN 后被错误降级为整文件 `200`，Worker 会取消该响应，并仅将这一次拖动以 `307` 回退到上游已经签名的 CDN 地址。正常起播仍保持中转，能正确返回 `206 Content-Range` 的线路不会触发回退。
...
- Emby 密码和 Access Token 保存在 D1；应限制 `ADMIN_TOKEN` 的传播范围。后台登录只保留在当前浏览器标签页，关闭标签页后需要重新登录。
- 正确配置不同的 `CF_DOMAIN` 和 `CF_DNS_DOMAIN` 后，上游网页无法从调度域同源访问后台 API；不要把两个变量设成同一个域名。
