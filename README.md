...
- 自动播放模式启用“优先直连”后，首次视频请求返回直连；同一客户端、视频和 Range 在 45 秒内重试且反代成功后，该客户端对当前节点将在 15 分钟内优先反代，换集和拖动不再重复等待直连失败。状态通过 Cookie、边缘 Cache API 和 isolate 内存保存，不写入 D1。
- 代理响应包含 `Server-Timing`，可查看 `ep-node`、`ep-route`、`ep-upstream`、`ep-redirects`、`ep-retries` 和 `ep-total`。
