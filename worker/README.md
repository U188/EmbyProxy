...
多个视频线路会根据真实请求的失败状态和首字节耗时被动调整顺序，不发起额外的视频竞速请求。响应中的 `Server-Timing` 可用于查看节点读取、自动路由、上游首字节、重定向、重试和 Worker 总处理时间。

自动模式启用“优先直连”时，首次视频请求返回 302；同一客户端、视频和 Range 在 45 秒内重试且反代获得成功响应后，Worker 会记住该客户端对当前节点的反代偏好 15 分钟。后续换集和拖动直接反代，状态到期后再尝试一次直连。状态通过 Cookie、边缘 Cache API 和 isolate 内存保存，不访问 D1；显式直连和显式中转模式不受影响。
